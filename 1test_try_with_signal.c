...
//		g_object_set (data.source, "pattern", 0, NULL);
		...
		if (!gst_pad_set_caps (source_src_pad, yuv_wh_caps)) {
				...
				GST_ELEMENT_ERROR (source, CORE, NEGOTIATION, (NULL),
								("Some debug information here"));
				return GST_FLOW_ERROR;
		}	

		GstPad *filter_sink_pad = gst_element_get_static_pad(filter, "sink");
		...
		GstPad *filter_src_pad = gst_element_get_static_pad(filter, "src");
		...
		GstPad *sink_sink_pad = gst_element_get_static_pad(sink, "sink");
